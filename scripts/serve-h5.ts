...
const optionsSchema = z.object({ host: z.string(), port: z.coerce.number().int().min(1).max(65535) });
const values: { host: string; port: string } = { host: "127.0.0.1", port: "4173" };
...
for (let index: number = 0; index < args.length; index += 2) {
  ...
  if ((name !== "--host" && name !== "--port") || value === undefined) throw new DomainError("INVALID_ARGUMENT", "Usage: node scripts/serve-h5.ts [--host 127.0.0.1|localhost] [--port 4173]", 400);
...
const server = await createH5Server({ workspaceRoot, dataRoot: path.join(workspaceRoot, "work", "h5-data"), now: (): string => new Date().toISOString() });
...
server.listen(options.port, host, (): void => { process.stdout.write(`${JSON.stringify({ event: "h5_listening", url: `http://${host}:${options.port}`, mode: "local-preview", dataDirectory: "work/h5-data", providersConnected: false })}\n`); });
